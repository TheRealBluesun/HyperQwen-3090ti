...
def _spec_attn_partial(
    ...
    tiles_per_seg = tl.maximum((tiles_total + NSEG - 1) // NSEG, MIN_TILES)
...
class SpecDecodeAttention:
    ...
    def run(self, q, key_cache, value_cache, out, cu_seqlens_q, seqused_k, block_table, scale,
        ...
        tile = getattr(self, "tile_override", None) or tile
        ...
        _spec_attn_partial[grid](
            ...
            *(v_scale_cache.stride() if quant else (0, 0, 0)),
            ...
            MIN_TILES=getattr(self, "min_tiles", 1),
            num_warps=getattr(self, "warps_override", None) or warps, num_stages=getattr(self, "stages", 1),
        )
