...
struct Params {
  ...
  float scale; int Hq, G, qmax, nseg, ntile, QT, window, causal;
};
...
__global__ void sda_combine_kernel(const float* part_o, const float* part_m, const float* part_l, __nv_bfloat16* out,
  ...
  if (i >= q_len) return;
  const long base = ((long)(req * Hq + h) * qmax + i) * nseg;
  float mmax = -INFINITY;
...
#undef L
  ...
  if (D == 256)
    sda_combine_kernel<256><<<cgrid, 256, 0, stream>>>(p.part_o, p.part_m, p.part_l, reinterpret_cast<__nv_bfloat16*>(out.data_ptr()), p.cu_q, out.stride(0), out.stride(1), Hq, (int)qmax, (int)nseg);
  else
    sda_combine_kernel<128><<<cgrid, 128, 0, stream>>>(p.part_o, p.part_m, p.part_l, reinterpret_cast<__nv_bfloat16*>(out.data_ptr()), p.cu_q, out.stride(0), out.stride(1), Hq, (int)qmax, (int)nseg);
}
