...
def unified_attention(
    ...
    import os as _os
    if max_seqlen_q > 1 and _os.environ.get("UA_BLOCK_M"):
        BLOCK_M = int(_os.environ["UA_BLOCK_M"]); BLOCK_Q = BLOCK_M // num_queries_per_kv
        launch_num_warps = int(_os.environ.get("UA_WARPS", "4")); launch_num_stages = int(_os.environ.get("UA_STAGES", "2"))
    ...
    if _os.environ.get("UA_TILE"):
        TILE_SIZE_PREFILL = int(_os.environ["UA_TILE"])
    if use_td:
