...
# Prefix-cache hits at 32-token granularity instead of the 864-token hybrid block (patch 17): each agent turn resumes
# within 32 tokens of the previous prompt. PREFIX_MATCH_UNIT=864 restores block-granular hits.
export EXTRA_ARGS='--prefix-match-unit '"${PREFIX_MATCH_UNIT:-32}"' --load-format=runai_streamer --model-loader-extra-config={"memory_limit":2542796800} --served-model-name Qwen3.8-27B qwen3.8-27b --default-chat-template-kwargs {"enable_thinking":false} '"${EXTRA_ARGS_ADD:-}"
